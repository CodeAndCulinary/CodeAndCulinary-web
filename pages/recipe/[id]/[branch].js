...
const RecipePage = ({ repositoryData, recipeData, recipeFound, id, branch, imageCache }) => {

    return (
        <>
            {recipeFound === true ? recipeHandler(recipeData, id, branch, imageCache) : notFound()}
        </>
    );
};
...
export async function getStaticProps({req, res, params}) {
    ...
    return { props: { repositoryData, recipeData, recipeFound, id, branch, imageCache }, revalidate: culinaryConfig.revalidationTime,}
  }
